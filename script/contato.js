...
document.addEventListener("DOMContentLoaded", function() {
    ...
    form.addEventListener("submit", function(event) {
        ...
        const mensagem = document.getElementById("input-massege").value;

        enviarMensagemWhatsApp(nome, telefone, email, mensagem);
    });
});

function enviarMensagemWhatsApp(nome, telefone, email, mensagem) {
    const numeroWhatsApp = "5585997272238"; // Seu número de telefone do WhatsApp
    const mensagemWhatsApp = `Olá, meu nome é ${nome}, meu telefone é ${telefone}, meu e-mail é ${email} e minha mensagem é: ${mensagem}`;
